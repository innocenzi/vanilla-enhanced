...
function byOptionalKey(value: JsonValue | undefined, keys: Record<string, number>, name: string): JsonValue | undefined {
  const key = keys[name];
  return key ? at(value, key) : undefined;
}
...
function spawnRecord(spawnTable: JsonValue | undefined, fallbackAreaId?: number): JsonRecord | undefined {
  const record = asRecord(spawnTable);
  if (record) return record;

  const table = asTable(spawnTable);
  const firstSpawnGroup = table && at(table, 1);
  if (!fallbackAreaId || !firstSpawnGroup) return undefined;

  return { [String(fallbackAreaId)]: firstSpawnGroup };
}
...
function addSpawns(
  ...
  const spawns = spawnRecord(spawnTable, fallbackAreaId);
  if (!spawns) return;
...
function resolveNpc(
  ...
  addSpawns(
    ...
    int(byOptionalKey(npc, db.keys.npcs, "zoneID")),
  );
}
...
function resolveObject(
  ...
  addSpawns(
    ...
    `Object ${objectId}`,
    allowUnmappedAreaIds,
    int(byOptionalKey(object, db.keys.objects, "zoneID")),
  );
}
...
function renderLocaleLua(db: NormalizedQuestieDb, references: BuildArtifacts["references"]): string | undefined {
  ...
  const lines: string[] = [
    ...
    `  meta = ${formatMeta({
      source: `Questie-derived ${expansion} ${locale} locale data`,
      questieRef: db.meta.questieRef,
      questieCommit: db.meta.questieCommit,
      expansion,
      locale,
