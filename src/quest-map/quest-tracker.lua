...
QuestMap.questTrackerButtons = QuestMap.questTrackerButtons or {}
...
local function AcquireTrackerButton(index)
    local button = QuestMap.questTrackerButtons[index]
    if button then
        return button
    end
    ...
    button:SetScript("OnClick", function(self)
        if self.questId then
            QuestMap:OpenQuestLogToQuest(self.questId)
        end
    end)
    QuestMap.questTrackerButtons[index] = button
    return button
end

local function HideUnusedButtons(firstUnusedIndex)
    for index = firstUnusedIndex, #QuestMap.questTrackerButtons do
        QuestMap.questTrackerButtons[index]:Hide()
        QuestMap.questTrackerButtons[index].questId = nil
    end
end

function QuestMap:RefreshQuestTrackerClicks()
